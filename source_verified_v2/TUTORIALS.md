...
## 4. Reconstruct retained CSVs without training

The [reconstruction companion](documents/ESI_20260925_Round2/Reconstruction/README.md) provides exact commands, optional dependencies, trace mapping and receipts. Run its utility against your STEAD HDF5 with a new output directory. The default checks regenerated byte streams and writes receipts only; `--write-csv` writes each file only if source ID, numerical waveform fingerprint and complete CSV hash agree. The complete recorded check matched all 2,234 records. A two-record smoke check alone does not validate a full collection.

After every file hash passes, the regenerated directory can supply `waveDir` for the existing replay commands. No new model inference was run for the reconstruction check. Historical selection decisions and hardware portability remain outside this verified scope.
