...
if stage=="oof"
 ...
 for i=1:min(N,maxRecords)
  ...
  for q=1:2
   ...
   assert(string(parts(q).event_id)==keys(ix));fresh=makeMeta(parts(q),cnn{k},tcn{k},cfg);expected=cache.M(ix,1);delta=max(abs(double(fresh{1})-double(expected{1})),[],'all');
