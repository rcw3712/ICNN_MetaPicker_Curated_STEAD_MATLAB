...
# Also verify the actual published test identities across all 70 output tables.
identities=None;sets=0
for job in csv.DictReader((root/'test_jobs.csv').open(encoding='utf-8')):
 file=root/job['expected']
...
assert len(identities)==330 and len({r[1] for r in identities})==221 and sets==70
