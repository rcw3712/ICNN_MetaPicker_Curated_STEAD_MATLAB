...
for i = 1:N
    ...
    for k = 1:factor
        ...
        if config.useAdditiveNoise
            ...
            active = 1:size(wf,2);
            if strcmpi(config.experimentMode,'Zonly'); active = 3; end
            sigPow = mean(wf(:,active).^2,'all');
            noisePow  = sigPow / (10^(snrTarget/10));
            noise     = sqrt(max(noisePow,0)) * randn(size(wf));
            if strcmpi(config.experimentMode,'Zonly'); noise(:,1:2)=0; end
            wf = wf + noise;
        end
        ...
        if strcmpi(config.experimentMode,'Zonly'); wf(:,1:2)=0; end
        dA.waveform      = wf;
