function summarize_strengthening(resultsRoot,frozenRoot)
% Retain meta-within-base structure. CIs are conditional on a fixed fit/split.
if nargin<2;frozenRoot='C:\Drive E\ICNN_submission_results\submission40_v5_oofstream';end
...
for b=42:44
 for s=42:44
  a=readtable(stackPath(b,"Full3C",s,'predictions'),'TextType','string');
  z=readtable(stackPath(b,"Zonly",s,'predictions'),'TextType','string');
  ...
  for mode=["Full3C","Zonly"]
   ...
   baseline=readtable(baselinePath(mode,b,'predictions'),'TextType','string');
